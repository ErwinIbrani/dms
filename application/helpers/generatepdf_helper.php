...
if (!function_exists('generate')) {
    ...
    function generateSurvey($model)
    {
        ...
        $raw_html      = '<!DOCTYPE html>
         ...
         $local_file = './uploads/surveysitac/' . $model['project_id'] . 'SITAC_SURVEY'.random_string('alnum', 16).'_'.$model['vendor_id'] . '.pdf';
         ...
         if (!$result) {
                ...
                $CI->CandidateDocument_Model->update($model['id'], [
                    'path' => $model['project_id'] . 'SITAC_SURVEY'.random_string('alnum', 16).'_'.$model['vendor_id'] . '.pdf']);
                //echo "HTTP Response: " . $http_response_header[0] . "<br/>";
                //echo($result);
            }
        }
